...
public:
  ...
  SerialPort(std::string port, speed_t baudrate, int timeout_ms = 5) {
    ...
    if (Init(port, baudrate) != 0) {
      LOG_WARN("[SerialPort] Init Error");
    }
  ...
  ssize_t recv(uint8_t *data, size_t len) {
    ...
    case -1: // error
      LOG_WARN("[SerialPort] communication error");
      break;
    case 0: // timeout
      // Allow Timeout
      // LOG_WARN("[SerialPort] Serial read timeout");
      break;
    default:
      recv_len = ::read(fd_, data, len);
      if (recv_len < 0) {
        LOG_WARN("[SerialPort] read data error");
      }
      break;
    }
    return recv_len;
...
private:
  int Init(std::string port, speed_t baudrate) {
    ...
    if (fd_ < 0) {
      LOG_WARN("[SerialPort] Open serial port %s failed", port.c_str());
      is_open = false;
      exit(-1);
    }
//    ioctl(fd_, TCFLSH, 0);
